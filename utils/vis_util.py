...
def visulize_result_2D(pc1, pc2, pc1_warp, num_pcs, args):
    SIDE_RANGE = (-50, 50)
    FWD_RANGE = (0, 100)
    RES = 0.15625 / 2
    ...
    for i in range(npcs1):
        im = cv2.circle(im, (x_img_w[i], y_img_w[i]), 2, (34, 139, 34), 2)
    ...
    im = cv2.putText(im, 'PC1_wrap', (600, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, (34, 139, 34), 2, cv2.LINE_AA)
...
def visulize_result_2D_seg_pre(pc1, pc2, mask, pred_m, num_pcs, args):
    ...
    ax1.scatter(pc_1[0, pred_m==0],pc_1[1,pred_m==0], s=6, c=np.array([[255/255, 99/255, 71/255]]))
    ax1.scatter(pc_1[0, pred_m==1],pc_1[1,pred_m==1], s=6, c=np.array([[65/255, 105/255, 225/255]]))
