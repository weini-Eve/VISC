    ...
    for i, data in tqdm(enumerate(eval_loader), total = len(eval_loader)):
        ...
        with torch.no_grad():
            ...
            if args.vis:
                ...
                visulize_result_2D(pc1, pc2, pc1_warp, num_pcs, args)
...
def plot_loss_epoch(train_items_iter, args, epoch):
    ...
    plt.clf()
    plt.plot(np.array(train_items_iter['selfLoss']).T, 'b')
    ...
    #plt.plot(np.array(train_items_iter['maskLoss']).T, 'y')
    #plt.plot(np.array(train_items_iter['motionLoss']).T, 'y')
    plt.plot(np.array(train_items_iter['totalLoss']).T, 'r')
    plt.legend(['selfLoss','chamferLoss','veloLoss','smoothnessLoss',\
        'opticalLoss','egoLoss', 'maskLoss','totalLoss'], loc='upper right')
    plt.xlabel('epoch')
    plt.ylabel('loss')
