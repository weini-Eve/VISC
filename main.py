...
##调用评估函数test or eval，并输出评估结果和内存占用情况
...
def test_vis(args, net, test_loader, textio):
    ...
    #args.vis_path_opt='checkpoints/'+args.exp_name+"/test_vis_opt/"
    ...
    #if not os.path.exists(args.vis_path_opt):
        #os.makedirs(args.vis_path_opt)
...
def train(args, net, train_loader, val_loader, textio):
    ...
    best_val_res = np.inf
    train_loss_ls = np.zeros((args.epochs)) # 用于记录每个训练轮数的训练损失
    val_score_ls = np.zeros(args.epochs) # 用于记录每个训练轮数的验证指标
    train_items_iter = {
                    'selfLoss': [],'smoothnessLoss': [],'veloLoss': [], 'chamferLoss': [],
                    'egoLoss':[], 'opticalLoss': [], 'totalLoss': [], 'maskLoss': [], 'motionLoss': []
                    } # 用于记录每个训练轮数的各项损失
    ...
    for epoch in range(args.epochs):
        ...
        train_loss_ls[epoch] = total_loss # 记录训练损失
        for it in loss_items:
            train_items_iter[it].extend([loss_items[it]]) # 记录各项损失
        ...
        eval_score = sf_metric['rne'] # 计算验证指标
        val_score_ls[epoch] = eval_score # 记录验证指标
        textio.cprint('mean RNE score: %f'%eval_score)
        
        if best_val_res >= eval_score:
            best_val_res = eval_score # 更新最佳验证指标
            textio.cprint('best val score till now: %f'%best_val_res)
            if torch.cuda.device_count() > 1:
                torch.save(net.module.state_dict(), 'checkpoints/%s/models/model.best.t7' % args.exp_name)
            else:
                torch.save(net.state_dict(), 'checkpoints/%s/models/model.best.t7' % args.exp_name) # 保存具有最佳验证指标的模型权重

        
        scheduler.step() # 调整学习率
        
        plot_loss_epoch(train_items_iter, args, epoch) # 绘制训练损失随训练轮数的变化
...
def main(io_args):
    ...
    # 将命令行输入的参数赋值给 args 对象，用于覆盖配置文件中解析得到的参数值
    ...
    # deterministic results 设置随机种子，用于保证训练过程的可重复性
...
if __name__ == '__main__': # 导入argparse模块，用于解析命令行参数
    #创建一个argparse.ArgumentParser对象，用于处理命令行参数的解析和帮助信息的生成
    ...
    parser.add_argument('--dataset_path', type= str, default = '/home/zyw/preprocess_res/flow_smp1/')
    parser.add_argument('--exp_name', type = str, default = 'cmval/cmval_noise')
    parser.add_argument('--model', type = str, default = 'cmflow')
    parser.add_argument('--dataset', type = str, default = 'vodDataset')
    # 解析命令行参数，将解析结果保存在args对象中
    args = parser.parse_args()
    # 调用main函数，将args作为参数传递给main函数
    main(args)